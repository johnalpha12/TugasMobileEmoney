 ...
 List<SavingGoal> savingGoals = [
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
              phoneNumber.replaceRange(4, phoneNumber.length - 2, '*' * (phoneNumber.length - 6)),
  ...
  void _navigateToEditSaving(int index) async {
    final updatedGoal = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => EditTabungan(goal: savingGoals[index])),
    );
          ...
          child: Column(
            ...
            children: [
              ...
              if (goal.targetDate != null)
                Padding(
                  padding: const EdgeInsets.only(top: 8),
                  child: Text("Target: ${goal.targetDate!.toIso8601String().split('T')[0]}"),
                ),
            ],
          ),
        ),
                    ...
                    TextButton(
                      onPressed: () {
                        if (_depositFormKey.currentState!.validate()) {
                          final tambah = int.parse(_depositController.text.replaceAll(',', ''));
                          setState(() {
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text("${(goal.progress * 100).toStringAsFixed(1)}%", style: const TextStyle(fontSize: 12)),
                  Text("Rp ${_formatCurrency(goal.amount.toInt())}", style: const TextStyle(fontSize: 12)),
                ],
              ),
            ],
          ),
  ...
  Widget _buildTransactionInfo(IconData icon, String label, double amount, Color color) {
    return Row(
      children: [
        ...
        children: [
          ...
          Center(
            child: Container(
              ...
              decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(10), color: Colors.white.withOpacity(0.1)),
              child: const Text("Personal", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            ),
          ),
          ...
          ListTile(
            leading: MouseRegion(
              ...
              child: GestureDetector(
                ...
                child: Stack(
                  ...
                  children: [
                    const CircleAvatar(radius: 30, backgroundImage: AssetImage('asset/foto.png')),
                    if (_showEditIcon)
                      Container(
                        width: 60,
                        height: 60,
            ...
            trailing: ElevatedButton(
              ...
              child: const Text("QR CODE", style: TextStyle(color: Colors.deepPurple, fontWeight: FontWeight.bold)),
            ),
          ),
                ...
                child: Column(
                  children: [
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text("Balance Kamu", style: TextStyle(color: Colors.grey, fontSize: 14)),
                            const SizedBox(height: 8),
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text("Rp ${_formatCurrency(balance.toInt())}", style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                                ElevatedButton(
                                  onPressed: () {},
                                  style: ElevatedButton.styleFrom(
                                    backgroundColor: Colors.deepPurple,
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                _buildTransactionInfo(Icons.arrow_upward, "Income", income, Colors.green),
                                _buildTransactionInfo(Icons.arrow_downward, "Expense", expense, Colors.red),
                              ],
                            ),
                          ],
                        ),
                    ...
                    const Align(
                      ...
                      child: Text("Tabungan Kamu", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ),
                    const SizedBox(height: 12),
                    ElevatedButton.icon(
                    ...
                    if (savingGoals.isEmpty)
                      Column(
                        children: [
                          ...
                          const SizedBox(height: 16),
                          Text("Belum ada tabungan", style: TextStyle(color: Colors.grey[600], fontSize: 16)),
                          const SizedBox(height: 8),
                          const Text("Tambahkan tabungan baru untuk memulai", style: TextStyle(color: Colors.grey)),
                        ],
                      )
                    else
                      Column(
                        children: List.generate(savingGoals.length, (index) => _buildGoalCard(index)),
                      ),
                  ],
                ),
              ),
